 ...
 *
 * @param state the [SirioPopupState] used to track whether the popup is visible
...
fun SirioPopup(
    ...
    expandedStates.targetState = state.isVisible
        ...
        Popup(
            ...
            SirioPopupContent(
                ...
                transformOrigin = TransformOrigin(
                    pivotFractionX = when (state.horizontalAlignment) {
                        Alignment.Start -> 1f
...
private data class SirioPopupPositionProvider(
    ...
    override fun calculatePosition(
        ...
        val isFitStart = (anchorBounds.right - contentOffsetX - popupContentSize.width) > 0
        ...
        val centerPlacementOffset = halfAnchor - popupHalfWidth + contentOffsetX
        ...
        val resultVertical = when {
            isTop && isFitTop -> true
            isTop && isFitBottom -> false
            !isTop && isFitBottom -> false
            !isTop && isFitTop -> true
            else -> isTop
        }

        val resultHorizontal = when (horizontalAlignment) {
            Alignment.Start -> if (isFitStart) Alignment.Start else if (isFitEnd) Alignment.End else if (isFitCenter) Alignment.CenterHorizontally else Alignment.Start
            Alignment.End -> if (isFitEnd) Alignment.End else if (isFitStart) Alignment.Start else if (isFitCenter) Alignment.CenterHorizontally else Alignment.End
            Alignment.CenterHorizontally -> if (isFitCenter) Alignment.CenterHorizontally else if (isFitEnd) Alignment.End else if (isFitStart) Alignment.Start else Alignment.CenterHorizontally
        ...
        val xStart = when (resultHorizontal) {
            Alignment.Start -> startPlacementOffset
            Alignment.CenterHorizontally -> centerPlacementOffset
            else -> endPlacementOffset
        }

        val yStart = if (resultVertical) toTop else toBottom

        Log.d("debug", "resultHorizontal: $resultHorizontal")
        Log.d("debug", "resultVertical: $resultVertical")
        Log.d("debug", "xStart: $xStart")
        Log.d("debug", "yStart: $yStart")

        onPopupPositionFound(resultHorizontal, resultVertical)
        return IntOffset(xStart, yStart)
    }
}
